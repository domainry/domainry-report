...
export preparation endpoints—together with their typed route/governance
contracts, authorization decisions, Object SQL rules, stable
...
Report handlers, route contracts, endpoint policies, or application/domain
...
route, governance, and request/response contracts. They are
...
Governed export clients should use Report's typed SDK request contracts and the
published browser client selected by the product. Operation prerequisites and
required headers are owned by the same Action/route definitions mounted by Runtime. Go verification
