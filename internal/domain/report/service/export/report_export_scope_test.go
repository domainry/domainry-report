...
func (*objectSQLExportAuthorizationStub) AuthorizeReportObjectSQLExport(context.Context, reportmodel.ReportSchema) error {
	return nil
}
...
func TestNormalizeObjectSQLExportScope(t *testing.T) {
	...
	scope, _, _, err := NormalizeScope(t.Context(), report, "orders", reportmodel.ReportExportControlSchema{SourceObjects: []string{"orders"}}, reportmodel.ReportExportScopeRequest{
	...
	if err := ValidateFieldAccess(t.Context(), report, reportmodel.ReportExportControlSchema{}, authorization); err != nil {
		t.Fatal(err)
	}
}
...
func TestNormalizeObjectSQLExportScopeRejectsUndeclaredSourceAndColumn(t *testing.T) {
	...
	authorization := &objectSQLExportAuthorizationStub{}
	if _, _, _, err := NormalizeScope(t.Context(), report, "orders", reportmodel.ReportExportControlSchema{}, reportmodel.ReportExportScopeRequest{Purpose: "audit"}, authorization); err == nil {
		t.Fatal("undeclared source accepted")
	}
	if _, _, _, err := NormalizeScope(t.Context(), report, "orders", reportmodel.ReportExportControlSchema{SourceObjects: []string{"orders"}}, reportmodel.ReportExportScopeRequest{Purpose: "audit", FieldProjection: []string{"missing"}}, authorization); err == nil {
